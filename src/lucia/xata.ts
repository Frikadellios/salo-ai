...
export class XataAdapter implements Adapter {
  ...
  public async getUserSessions(user_id: UserId): Promise<DatabaseSession[]> {
    const records = await this.controller.db.session
      .filter({ user_id })
      .getAll()
  ...
  public async updateSessionExpiration(
    ...
    await this.controller
      .sql`UPDATE "session" SET expires_at=${expiresAt} WHERE id=${sessionId}`
  }

  public async deleteExpiredSessions(): Promise<void> {
    await this.controller
      .sql`DELETE FROM "session" WHERE expires_at <=${new Date()}`
  }
  ...
  private async getUserFromSessionId(
    ...
    const theSession = await this.controller.db.session
      .filter({ session_id })
      .getFirst()
    if (theSession) {
      const { user_id } = theSession
      const getUser = await this.controller.db.user
        .filter({ user_id })
        .getFirst()
